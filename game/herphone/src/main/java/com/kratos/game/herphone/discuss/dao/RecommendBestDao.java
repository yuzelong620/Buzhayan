...
@Repository
public class RecommendBestDao extends BaseDao<RecommendBestEntity>{
	 ...
	 * 查询未处理推荐评论表
	 * */
	public List<RecommendBestEntity> findByUndeal(int page,int count){
	 ...
	 *查询搁置状态的评论
	 * */
	public List<RecommendBestEntity> findByhold(int page,int count){
		int offset = (page - 1) * count;
		int limit = count;
	 ...
	 * 根据playerid查询信息
	 * */
	public List<RecommendBestEntity> findByIds(List<Long> playerid) {
	...
	public void setRecommendUtd(String discussId) {
		Criteria criteria = new Criteria(_ID).in(discussId);
		Query query = new Query(criteria);
		Update update = new Update();
